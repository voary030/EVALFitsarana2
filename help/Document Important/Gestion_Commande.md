...
1. [[#1. Contexte métier (Fichier 3 — champ `etat`)|1. Contexte métier (Fichier 3 — champ `etat`)]]
2. [[#2. Endpoint|2. Endpoint]]
	1. [[#2. Endpoint#Méthodes supportées|Méthodes supportées]]
3. [[#3. Champs principaux de la ressource `orders`|3. Champs principaux de la ressource `orders`]]
4. [[#4. Processus de transformation panier → commande|4. Processus de transformation panier → commande]]
5. [[#5. Schéma XML — Création d'une commande (POST)|5. Schéma XML — Création d'une commande (POST)]]
6. [[#6. Schéma XML — Réponse après création (201 Created)|6. Schéma XML — Réponse après création (201 Created)]]
7. [[#7. Schéma JSON — Réponse GET (output_format=JSON)|7. Schéma JSON — Réponse GET (output_format=JSON)]]
8. [[#8. Mise à jour de l'état — `order_histories`|8. Mise à jour de l'état — `order_histories`]]
	1. [[#8. Mise à jour de l'état — `order_histories`#Champs de `order_histories`|Champs de `order_histories`]]
	2. [[#8. Mise à jour de l'état — `order_histories`#Schéma XML — Création d'un historique (POST)|Schéma XML — Création d'un historique (POST)]]
	3. [[#8. Mise à jour de l'état — `order_histories`#Schéma JSON — Réponse (output_format=JSON)|Schéma JSON — Réponse (output_format=JSON)]]
9. [[#9. États de commande disponibles|9. États de commande disponibles]]
10. [[#10. Mapping Fichier 3 → états|10. Mapping Fichier 3 → états]]
