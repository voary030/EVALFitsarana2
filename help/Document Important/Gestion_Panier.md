...
1. [[#1. Contexte métier (Fichier 3 — champ `achat`)|1. Contexte métier (Fichier 3 — champ `achat`)]]
2. [[#2. Endpoint|2. Endpoint]]
	1. [[#2. Endpoint#Méthodes supportées|Méthodes supportées]]
3. [[#3. Champs principaux de la ressource `carts`|3. Champs principaux de la ressource `carts`]]
4. [[#4. Pré-requis avant création d'un panier|4. Pré-requis avant création d'un panier]]
5. [[#5. Étape 1 — Créer le panier (POST)|5. Étape 1 — Créer le panier (POST)]]
6. [[#6. Étape 2 — Ajouter des produits (cart_rows) via PUT|6. Étape 2 — Ajouter des produits (cart_rows) via PUT]]
	1. [[#6. Étape 2 — Ajouter des produits (cart_rows) via PUT#Structure de `cart_rows`|Structure de `cart_rows`]]
7. [[#7. Exemples complets tirés du Fichier 3|7. Exemples complets tirés du Fichier 3]]
	1. [[#7. Exemples complets tirés du Fichier 3#Commande 1 — Rakoto : T_01 × 3 (ngoza)|Commande 1 — Rakoto : T_01 × 3 (ngoza)]]
	2. [[#7. Exemples complets tirés du Fichier 3#Commande 2 — Rajao : T_01 × 2 (kely) + C_03 × 1|Commande 2 — Rajao : T_01 × 2 (kely) + C_03 × 1]]
8. [[#8. Schéma JSON — Réponse GET (output_format=JSON)|8. Schéma JSON — Réponse GET (output_format=JSON)]]
9. [[#9. Lier un panier à un client — Récapitulatif|9. Lier un panier à un client — Récapitulatif]]
